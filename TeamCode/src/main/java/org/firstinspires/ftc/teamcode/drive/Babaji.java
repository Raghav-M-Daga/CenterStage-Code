 ...
 */
@Config
public class Babaji extends MecanumDrive {
    ...
    public static double backdropX = 10, backdropY = -25, backdropAngle = 0, backdropInterpol = -45;
    public static double intakeX = -90, intakeY = -105, intakeAngle = 270;

    public Babaji(HardwareMap hardwareMap) {
...
//        outtake.setJonnyRotate(Outtake.JonnyRotatePos.VERTICAL);
    ...
    public void travelToOuttake(Pose2d currPos) {
        TrajectorySequence traverseField = this.trajectorySequenceBuilder(currPos)
                .setReversed(true)
                .splineToLinearHeading(new Pose2d(backdropX, backdropY, Math.toRadians(backdropAngle)), Math.toRadians(backdropInterpol))
                .build();
        this.followTrajectorySequenceAsync(traverseField);
    }

    public void travelToIntake(Pose2d currPos) {
        TrajectorySequence traverseField = this.trajectorySequenceBuilder(currPos)
                .setReversed(false)
                .splineToLinearHeading(new Pose2d(intakeX, intakeY, intakeAngle), Math.toRadians(intakeAngle))
                .build();
        this.followTrajectorySequenceAsync(traverseField);
    }
